...
public class EarthquakeCityMap extends PApplet {
  ...
  private void sortAndPrint(int numToPrint) {
    ...
    // method 1: uses List.add()
    List<EarthquakeMarker> quakeList = new ArrayList<EarthquakeMarker>();
    for (Marker marker : quakeMarkers) {
      quakeList.add((EarthquakeMarker)marker);
    }
    Collections.sort(quakeList);
    
    // method 2: uses Arrays.toList(array);
    // 2.1: converts `quakeMarkers` to an array whose type is `EarthquakeMarker[]`
    //   - `quakeMarkers`: private field, or private member variable
    //   - cannot: EarthquakeMarker[] = (EarthquakeMarker[]) `Object[]`;
    //   - can: lets toArray() returns `EarthquakeMarker[]` directly
//    EarthquakeMarker[] temp = new EarthquakeMarker[quakeMarkers.size()];
//    quakeMarkers.toArray(temp);
    
    // 2.2: constructs an List<EarthquakeMarker> from an EarthquakeMarker array
//    List<EarthquakeMarker> quakeList = new ArrayList<EarthquakeMarker>(Arrays.asList(temp));
    
    // 2.3: sorts this list
//    Collections.sort(quakeList); // List<EarthquakeMarker>
    // can: ArrayList<EarthquakeMarker> -> List<EarthquakeMarker>
    // cannot ArrayList<EarthquakeMarker> -> ArrayList<Marker>
    // List<EarthquakeMarker> xx = quakeMarkers

    EarthquakeMarker[] list = new EarthquakeMarker[quakeList.size()];
    quakeList.toArray(list);
    for (int i = 0; i <= Math.min(numToPrint - 1, list.length - 1); i++) {
