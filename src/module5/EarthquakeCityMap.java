...
public class EarthquakeCityMap extends PApplet {
  ...
  public void mouseClicked() {
    ...
    if (lastClicked != null) {
      ...
      clickedOnQuake(quakeMarkers);
      if (lastClicked == null) {
        clickedOnCity(cityMarkers);
      }
    }
  }
    ...
    for (Marker m : markers) {
      if (m.isInside(map, mouseX, mouseY)) {
        ...
        for (Marker qm : quakeMarkers) {
          if (qm.getDistanceTo(lastClicked.getLocation()) > ((EarthquakeMarker) qm).threatCircle()) {
            qm.setHidden(true);
          }
        }
      }
  ...
  private void clickedOnQuake(List<Marker> markers) {
    for (Marker m : markers) {
      if (((EarthquakeMarker) m).isInside(map, mouseX, mouseY)) {
        ...
        for (Marker qm : cityMarkers) {
          if (qm.getDistanceTo(m.getLocation()) > ((EarthquakeMarker) m).threatCircle()) {
            qm.setHidden(true);
          }
        }
      }
